...
// count actas to compute next numero
$cnt = $pdo->query("SELECT COUNT(*) c FROM actas")->fetch()['c'];
$numero_sugerido = numero_acta_for_index($cnt);

function vend_label($c){ $m = vendedores_map(); return isset($m[$c]) ? $c.' - '.$m[$c] : $c; }
?>
...
header{background:#fff;padding:12px;border-bottom:1px solid #eee;display:flex;align-items:center;gap:12px}
header img{height:56px}
.container{max-width:1100px;margin:20px auto}
.card{border-radius:10px;padding:12px;background:#fff;border:1px solid #e6e6e6}
.small{color:#64748b}
</style>
</head><body>
...
<script>
// Autocompletar cliente por código (fetch from server via simple ajax)
